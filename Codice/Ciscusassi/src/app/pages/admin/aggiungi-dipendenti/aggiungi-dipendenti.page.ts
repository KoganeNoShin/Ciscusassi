...
export class AggiungiDipendentiPage implements OnInit {
  ...
  aggiungiDipendente() {
    if (this.password.length < 8) {
      this.presentToast('La password deve contenere almeno 8 caratteri.', 'danger');
      return;
    }
    ...
    this.impiegatoService.AddImpiegato(nuovoDipendente).subscribe({
      next: (response) => {
        ...
        this.presentToast('Dipendente aggiunto con successo!', 'success');
        this.resetForm();
      },
      error: (error) => {
        console.error('Errore durante l\'aggiunta del dipendente:', error);
        this.presentToast('Errore durante l\'aggiunta del dipendente.', 'danger');
      }
    });
  }
