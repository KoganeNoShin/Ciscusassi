	...
	try {
		...
		for (let i = 0; i < asporti.length; i++) {
			for (let j = 0; j < faker.number.int({ min: 3, max: 8 }); j++) {
				...
				try {
					await asp_prod.create({
						ref_asporto: ref_asporto,
						ref_prodotto: ref_prodotto,
					});
					console.log(
						`🛍️  Aggiunto il prodotto con ID ${ref_prodotto} nell'ordine di asporto con ID ${ref_asporto}!`
					);
				} catch (err) {
					console.log(
						`❌ Impossibile aggiungere il prodotto con ID ${ref_prodotto} nell'ordine di asporto con ID ${ref_asporto}: ${err}`
					);
					throw err;
				}
			}
		...
		console.log(
			`❌ Errore durante la generazione dei prodotti comprati negli ordini d'asporto:`,
			err
		);
		throw err;
	}
