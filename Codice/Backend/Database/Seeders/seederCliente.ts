...
async function generateCliente(count: number): Promise<string> {
    ...
    try {
        ...
        for (let i = 0; i < count; i++) {
            ...
            try {
                await cliente.create({ nome: nome, cognome: cognome, email: email, password: password, punti: punti, data_nascita: data_nascita, image: imageBase64 });
                console.log(`👱 ${nome} ${cognome} è diventato nostro cliente!`);
            } catch (err) {
                console.error(`💀 ${nome} ${cognome} ci ha lasciato prematuramente. Causa di morte: ${err}`);
                throw err;
            }
        }
        ...
        console.error(`💀 Errore durante la generazione dei clienti. Causa: ${err}`);
        throw err;
    }
}
