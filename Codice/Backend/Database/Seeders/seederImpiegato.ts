...
export async function generateImpiegato(): Promise<string> {
    ...
    try {
        ...
        for (const f of filiali) {
            ...
            try {
                await impiegato.create({ nome: admin_nome, cognome: admin_cognome, ruolo: admin_ruolo, foto: admin_imageBase64, password: admin_password, email: admin_email, data_nascita: admin_data_nascita, ref_filiale: f.id_filiale })
                console.log(`💼 ${admin_nome} ${admin_cognome} è stato assunto in ${f.indirizzo} come ${admin_ruolo}!`);
            }
            catch (err) {
                console.error(`🤡 ${admin_nome} ${admin_cognome} è stato licenziato! Causa di licenziamento: ${err}`);
                throw err;
            }
            ...
            try {
                await impiegato.create({ nome: chef_nome, cognome: chef_cognome, ruolo: chef_ruolo, foto: chef_imageBase64, password: chef_password, email: chef_email, data_nascita: chef_data_nascita, ref_filiale: f.id_filiale });
                console.log(`🥩 ${chef_nome} ${chef_cognome} è stato assunto in ${f.indirizzo} come ${chef_ruolo}!`);
            }
            catch (err) {
                console.error(`🤡 ${chef_nome} ${chef_cognome} è stato licenziato! Causa di licenziamento: ${err}`);
                throw err;
            }
            ...
            for (let i = 0; i < nCamerieri; i++) {
                ...
                try {
                    await impiegato.create({ nome: waiter_nome, cognome: waiter_cognome, ruolo: waiter_ruolo, foto: waiter_imageBase64, password: waiter_password, email: waiter_email, data_nascita: waiter_data_nascita, ref_filiale: f.id_filiale });
                    console.log(`🍽️  ${waiter_nome} ${waiter_cognome} è stato assunto in ${f.indirizzo} come ${waiter_ruolo}!`);
                } catch (err) {
                    console.error(`🤡 ${waiter_nome} ${waiter_cognome} è stato licenziato! Causa di licenziamento: ${err}`);
                    throw err;
                }
