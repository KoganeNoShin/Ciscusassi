...
class OrdineService {
	static async creaOrdine(data: OrdineInput): Promise<number> {
		try {
			if (data.ref_cliente != null) {
				const cliente = await Cliente.getByNumeroCarta(
					data.ref_cliente
				);
				...
				const [nome, cognome, anno] =
					data.username_ordinante.split('.');
				const annoNascitaCliente = cliente.data_nascita.split('-')[0];

				const nomeMatch =
					cliente.nome.toLowerCase().replace(' ', '') ===
					nome.toLowerCase().replace(' ', '');
				const cognomeMatch =
					cliente.cognome.toLowerCase().replace(' ', '') ===
					cognome.toLowerCase().replace(' ', '');
				const annoMatch = annoNascitaCliente === anno;

				if (!(nomeMatch && cognomeMatch && annoMatch)) {
					throw new Error(
						'Username non corrisponde ai dati del cliente'
					);
				}
			}

			const ordineEsistente =
				await Ordine.getOrdineByPrenotazioneAndUsername(
			...
			if (ordineEsistente) {
				throw new Error(
					'Esiste già un ordine con questo username per la stessa prenotazione'
				);
			}
			...
			console.error(
				"❌ [OrdineService] Errore durante la creazione dell'ordine:",
				error
			);
			throw error;
		}
	...
	static async aggiungiPagamento(
		i: number,
		d: string,
		id_ordine: number
	): Promise<void> {
		try {
			...
			const importo_minimo = await this.calcolaImportoTotale(
				id_ordine,
				true
			);
			...
			console.error(
				"❌ [OrdineService] Errore durante l'aggiunta del pagamento all'ordine:",
				error
			);
			throw error;
		}
	...
	static async eliminaOrdine(id: number): Promise<void> {
		try {
			...
			console.error(
				"❌ [OrdineService] Errore durante l'eliminazione dell'ordine:",
				error
			);
			throw error;
		}
	...
	static async getOrdineById(id: number): Promise<OrdineRecord | null> {
		try {
			...
			console.error(
				"❌ [OrdineService] Errore durante il recupero dell'ordine:",
				error
			);
			throw error;
		}
	...
	static async getAllOrdini(): Promise<OrdineRecord[]> {
		try {
			...
			console.error(
				'❌ [OrdineService] Errore durante il recupero degli ordini:',
				error
			);
			throw error;
		}
	...
	static async getOrdiniByCliente(
		clienteId: number
	): Promise<OrdineRecord[] | null> {
		try {
			return await Ordine.getByCliente(clienteId);
		} catch (error) {
			console.error(
				'❌ [OrdineService] Errore durante il recupero degli ordini per cliente:',
				error
			);
			throw error;
		}
	...
	static async getOrdiniByPrenotazione(
		...
		try {
			...
			console.error(
				'❌ [OrdineService] Errore durante il recupero degli ordini per prenotazione:',
				error
			);
			throw error;
		}
	...
	static async getOrdineByPrenotazioneAndUsername(
		prenotazioneID: number,
		username: string
	): Promise<number | null> {
		try {
			return await Ordine.getOrdineByPrenotazioneAndUsername(
				prenotazioneID,
				username
			);
		} catch (error) {
			console.error(
				"❌ [OrdineService] Errore durante il recupero dell'ordine per prenotazione ed Username:",
				error
			);
			throw error;
		}
	...
	static async calcolaImportoTotale(
		ordineId: number,
		pagamento: boolean
	): Promise<number> {
		...
		const prenotazione = await Prenotazione.getById(
			ordine.ref_prenotazione
		);
		if (!prenotazione) {
			throw new Error(
				`Prenotazione con ID ${ordine.ref_prenotazione} non trovata per ordine`
			);
		}
		// Prodotti non alla romana dell'ordine
		const prodottiNotRomana = await OrdProd.getByOrdineAndRomana(
			ordineId,
			false
		);
		...
		if (prodottiNotRomana != null && prodottiNotRomana.length !== 0) {
			for (const prod_asp of prodottiNotRomana) {
				...
				if (!prodotto) {
					throw new Error(
						`Prodotto con ID ${prod_asp.ref_prodotto} non trovato`
					);
				}
		...
		const prodottiRomana = await OrdProd.getByOrdineAndRomana(
			ordineId,
			true
		);
		let totaleRomana = 0;
		...
		if (ordine.ref_cliente != null) {
			...
			if (pagamento) {
				// Cambia i punti solo se paga
				console.log(
					`${ordine.ref_cliente} ${ordine.username_ordinante} nuovi punti: ${punti}`
				);
