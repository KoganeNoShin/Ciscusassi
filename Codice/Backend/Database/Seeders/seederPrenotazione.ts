	...
	try {
		...
		for (let i = 0; i < count; i++) {
			...
			try {
				...
				console.log(
					`📆  Aggiunta la prenotazione di ${numero_persone} persone giorno ${data_ora_prenotazione}. Gli è stata assegnata la torretta ${ref_torretta} ed è stata ${ref_cliente == null ? 'creata in loco.' : `creata online dall'utente ${ref_cliente}.`}`
				);
			} catch (err) {
				console.log(
